...
**Consequence:** Manual URL routing, manual JSON parsing, no middleware. But the server is a single file with zero external dependencies.
...
**Consequence:** Ollama binaries per platform (varies from ~200MB to ~3.5GB depending on GPU runner inclusion). Four copies needed (Mac ARM, Mac Intel, Windows, Linux x86_64).
...
**Decision:** All cryptographic operations (chat encryption, Locked file storage) use a single master password. No per-file passwords.
...
**Implementation:** Browser-side AES-256-GCM via `window.crypto.subtle` for chat history. Server-side native AES-256-GCM/CTR via `filecrypt.py` (libcrypto ctypes) for Locked file operations. Both use the same user-provided master password.
...
## ADR-011: Piper TTS Decommission

**Decision:** Remove Piper WASM neural TTS and use browser-native `SpeechSynthesis` exclusively.

**Rationale:** Piper WASM introduced a 90MB asset payload and 20-30s latency before audio playback began. The WASM runtime also loaded assets from hardcoded CDN paths, violating the air-gap requirement. Browser-native `SpeechSynthesis` plays instantly with zero dependencies.

**Consequence:** Voice quality varies by OS/browser (macOS voices are good, Windows/Linux are more robotic). But the trade-off is justified: instant playback vs. 30s latency, zero dependencies vs. 90MB assets, and guaranteed air-gap compliance.

---

## ADR-012: Single-Radio COMMS Gateway

**Decision:** Support exactly one Meshtastic radio per Blackout Drive instance.

**Rationale:** The drive functions as a basecamp gateway — it connects to the mesh through a single serial radio and provides AI dispatch, encrypted logging, and tactical telemetry to the operator. Multiple radios per drive would add complexity with minimal benefit (a single LoRa radio already reaches the full mesh).

**Consequence:** One `CommsManager` → one `SerialIO` → one serial port. Additional radios on the mesh are peer nodes, not additional drive connections. Multi-radio support may be considered for V2 if a use case emerges.
