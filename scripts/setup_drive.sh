...
if [ ! -f "$DRIVE_DIR/models/$BLACKOUT_MODEL_FILE" ]; then
    bash "$SCRIPT_DIR/download_models.sh"
else
    SIZE=$(du -sh "$DRIVE_DIR/models/$BLACKOUT_MODEL_FILE" | cut -f1)
    echo "  Model already present: $BLACKOUT_MODEL_FILE ($SIZE)"
fi
echo ""
...
check "drive/STOP_BEACON.command"  "$DRIVE_DIR/STOP_BEACON.command"
check "drive/STOP_BEACON.bat"      "$DRIVE_DIR/STOP_BEACON.bat"
...
# AI model
check "models/$BLACKOUT_MODEL_FILE" "$DRIVE_DIR/models/$BLACKOUT_MODEL_FILE" "2000"
