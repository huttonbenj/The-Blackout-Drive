...
- **Encryption weaknesses** — Issues in AES-256-GCM chat encryption or BKV file encryption (AES-256-GCM for files <50MB; AES-256-CTR + HMAC-SHA256 for files ≥50MB)
...
- Issues in third-party dependencies (Ollama, Meshtastic/pyserial) — report those upstream
...
- **No unsolicited external network calls** — the server makes zero automatic outbound requests; the one optional update check only fires when the user explicitly triggers it from Settings, and only transmits the version string
