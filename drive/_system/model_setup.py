...
def has_dedicated_gpu():
    """Detect if the system has a dedicated/discrete GPU (NVIDIA or AMD) capable of heavy ML.
    Returns (bool, str): (has_gpu, gpu_name_string)."""
    try:
        if sys.platform == 'darwin':
            # Apple Silicon (M1/M2/M3) has unified memory with massive bandwidth.
            # Treat as "dedicated" equivalent for ML purposes so RAM dictates tier.
            import subprocess
            try:
            ...
            import subprocess
            output = ''
            # Try wmic first (available on Windows 10, deprecated on 11)
            try:
                ...
                pass
            # Fallback to PowerShell Get-CimInstance (works on all modern Windows)
            if not output.strip() or output.strip().lower() == 'name':
                try:
                    output = subprocess.check_output(
                        'powershell -NoProfile -Command "Get-CimInstance Win32_VideoController | Select-Object -ExpandProperty Name"',
                        shell=True, stderr=subprocess.STDOUT, text=True, timeout=10
                    )
                except Exception:
                    pass
            # Parse GPU names from output (skip header line if wmic)
            gpu_names = [line.strip() for line in output.strip().splitlines()
                         if line.strip() and line.strip().lower() != 'name']
            gpu_str = '; '.join(gpu_names) if gpu_names else 'Unknown'

            keywords = ['nvidia', 'rtx ', 'gtx ', 'rx ']
            for kw in keywords:
                if kw in output.lower():
                    return True, gpu_str
            return False, gpu_str
            ...
            gpu_str = output.strip() or 'Unknown'
...
def select_model_key(manifest, script_dir, auto_detect=False):
    ...
    # Priority 2: Auto-detect from RAM and GPU
    if auto_detect:
        ram_gb = get_system_ram_gb()
        has_gpu, _gpu_name = has_dedicated_gpu()
        threshold = manifest.get('ramThresholdGB', 9)
        
        # Max tier requires both >= 9GB RAM AND a dedicated GPU (except on Mac)
        if ram_gb >= threshold and has_gpu:
            detected_tier = 'max'
        else:
            detected_tier = 'base'
            
        print(f"Hardware detected: {ram_gb:.1f} GB RAM, Dedicated GPU: {has_gpu} -> auto tier: {detected_tier}",
              file=sys.stderr)
...
def check_hardware_sufficient(ram_gb, has_gpu, is_mac):
    """Determine if the hardware can run BEACON AI.
    Returns (ai_disabled: bool, reason: str or None).
    Reasons: 'no_gpu', 'insufficient_ram', None (capable)."""
    MIN_RAM_GB = 7.5  # 8GB minus OS overhead reporting variance
    if ram_gb > 0 and ram_gb < MIN_RAM_GB:
        return True, 'insufficient_ram'
    if not is_mac and not has_gpu:
        return True, 'no_gpu'
    return False, None


def write_active_tier(script_dir, tier, model_key, model_name, source,
                     ram_gb=None, has_gpu=None, gpu_name=None,
                     ai_disabled=False, ai_disabled_reason=None):
    """Write the active tier info to data/active_tier.json for diagnostics and UI."""
    ...
    info = {
        ...
        'source': source,
        'detectedRamGB': round(ram_gb, 1) if ram_gb is not None else None,
        'hasGpu': has_gpu,
        'gpuName': gpu_name,
        'aiDisabled': ai_disabled,
        'aiDisabledReason': ai_disabled_reason,
    }
...
def main():
    ...
    # Always detect hardware for diagnostics and gating
    ram_gb = get_system_ram_gb()
    has_gpu, gpu_name = has_dedicated_gpu()
    is_mac = sys.platform == 'darwin'
    ai_disabled, ai_disabled_reason = check_hardware_sufficient(ram_gb, has_gpu, is_mac)

    if ai_disabled:
        print(f"AI DISABLED: reason={ai_disabled_reason}, RAM={ram_gb:.1f}GB, "
              f"GPU={gpu_name}, hasDedicatedGPU={has_gpu}", file=sys.stderr)
    else:
        print(f"Hardware OK: RAM={ram_gb:.1f}GB, GPU={gpu_name}, "
              f"hasDedicatedGPU={has_gpu}", file=sys.stderr)

    # Write active tier info for diagnostics and UI hardware gating
    write_active_tier(script_dir, tier, model_key, model_display, source,
                     ram_gb=ram_gb, has_gpu=has_gpu, gpu_name=gpu_name,
                     ai_disabled=ai_disabled, ai_disabled_reason=ai_disabled_reason)
    ...
    if '--print-config' in actions:
        ...
        print(f"AI_DISABLED={'1' if ai_disabled else '0'}")
        print(f"AI_DISABLED_REASON={ai_disabled_reason or ''}")
        ...
        log_dir = os.path.join(os.path.dirname(script_dir), 'USER_DATA', 'logs')
