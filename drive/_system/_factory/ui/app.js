...
if (chatContainer) {
  chatContainer.addEventListener('scroll', () => {
    ...
    _isUserScrolledUp = distFromBottom > 150;
  }, { passive: true });
}
...
const OVERLAY_TIMEOUT_MS = 300000; // 5 minutes max (for very slow computers compiling shaders)
...
function showConnectingOverlay() {
  ...
  overlay.innerHTML = `
    ...
    <div class="connecting-sub">Loading AI model into memory. This runs entirely on your hardware — speed depends on your system.</div>
...
async function checkConnection() {
  try {
    // Step 1: Quick connectivity check — is Ollama even running?
    const psRes = await fetch(`${CONFIG.ollamaHost}/api/ps`, {
      signal: AbortSignal.timeout(2500)
    });
    if (!psRes.ok) return 'error';

    // Step 2: Check if model is at least registered in /api/ps
    const psData = await psRes.json().catch(() => null);
    if (!psData || !Array.isArray(psData.models)) return 'error';
    const modelName = (CONFIG.model || 'blackout-beacon').split(':')[0];
    const hasModel = psData.models.some(m => (m.name || '').split(':')[0] === modelName);

    // If the model isn't even listed in ps yet, it's still being imported/loaded.
    if (!hasModel) return 'warming_up';

    // Step 3 — Phase 1: Raw generate-ping (fast, no system prompt).
    // Confirms the model runner is alive and Vulkan shaders are compiled.
    // Uses raw:true so it's a tiny 2-token request (~200ms when warm).
    // The timeout MUST be large (60s) because on first run, allocating memory
    // and compiling Vulkan shaders can take 15-30+ seconds on slow hardware.
    // If the timeout is too short, the UI aborts the request, which causes
    // Ollama to abort the model load, creating an infinite loop of failures.
    const genRes = await fetch(`${CONFIG.ollamaHost}/api/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: CONFIG.model || 'blackout-beacon',
        prompt: 'hi',
        raw: true,
        stream: false,
    ...
    if (!genRes.ok) return 'warming_up';

    // Step 4 — Phase 2: System prompt cache priming.
    // The raw ping bypasses the system prompt, so the KV cache is empty.
    // If we dismiss the overlay now, the user's first real chat would trigger
    // a 12s+ system prompt eval that can return 500 on slow hardware.
    // Send a /api/chat request with a tiny user message to force Ollama to
    // evaluate and cache the full system prompt from the Modelfile.
    // 60s timeout — we know the runner is alive, this is purely prompt eval.
    const chatRes = await fetch(`${CONFIG.ollamaHost}/api/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: CONFIG.model || 'blackout-beacon',
    ...
    return chatRes.ok ? 'online' : 'warming_up';
  } catch {
    // Distinguish between "Ollama not running" and "model still loading"
    try {
      const probe = await fetch(`${CONFIG.ollamaHost}/api/ps`, {
        signal: AbortSignal.timeout(1500)
      });
      if (probe.ok) return 'warming_up'; // Ollama is up, model not ready
    } catch {}
    return 'error'; // Ollama not running at all
  }
}

/**
 * Lightweight connection check — used ONLY after the model has been confirmed
 * online via generate-ping. Just verifies Ollama is still running and the
 * model is still registered. Does NOT send any generate requests.
 */
async function _checkConnectionLight() {
  try {
    ...
    const modelName = (CONFIG.model || 'blackout-beacon').split(':')[0];
    const hasModel = data.models.some(m => (m.name || '').split(':')[0] === modelName);
    return hasModel ? 'online' : 'warming_up';
  } catch {
    return 'error';
  }
}
...
function setStatus(state) {
  ...
  if (state === 'online') {
    ...
    if (welcomeTitle) {
      ...
      welcomeTitle.style.color = 'var(--amber-dim)';
    }
    ...
    showWarning(errMsg, 'info', false);

  } else {
    statusText.textContent = 'BEACON:STARTING';
    sendBtn.disabled = true;
    if (welcomeTitle) { welcomeTitle.textContent = 'BEACON:STARTING'; welcomeTitle.style.color = 'var(--amber-dim)'; }
  }
}
...
const _warmupTips = [
  'BEACON runs entirely offline — no internet, no cloud, no tracking.',
  'Your conversations can be encrypted with AES-256-GCM — enable Blackout Protocol for automatic encryption.',
  ...
  'BEACON runs locally — not in the cloud. The AI model loads into memory each launch, then runs at the speed your hardware allows.',
...
// ── Hardware Insufficient Overlay ─────────────────────────────
// Shown permanently when the host machine lacks a dedicated GPU
// or has insufficient RAM to run the AI engine.
let _hardwareInsufficient = false;
let _hardwareInsufficientReason = null; // 'no_gpu' | 'insufficient_ram' | null — cached for comms.js

function _showHardwareInsufficientOverlay(reason, engineInfo) {
  _hardwareInsufficient = true;
  _hardwareInsufficientReason = reason || null;

  // Build dynamic reason text with actual hardware specs
  const ramGB = engineInfo?.detectedRamGB ? `${engineInfo.detectedRamGB} GB` : 'Unknown';
  const gpuName = engineInfo?.gpuName || 'Unknown';

  let reasonTitle, reasonDetail, specLines;
  if (reason === 'insufficient_ram') {
    reasonTitle = 'INSUFFICIENT MEMORY';
    reasonDetail = 'BEACON AI requires at least 8 GB of RAM to load the language model into memory.';
    specLines = `<div class="hw-spec-line"><span class="hw-spec-label">DETECTED RAM</span><span class="hw-spec-val hw-spec-fail">${ramGB}</span></div>
                 <div class="hw-spec-line"><span class="hw-spec-label">REQUIRED RAM</span><span class="hw-spec-val">8 GB minimum</span></div>
                 <div class="hw-spec-line"><span class="hw-spec-label">GPU</span><span class="hw-spec-val">${gpuName}</span></div>`;
  } else {
    reasonTitle = 'NO DEDICATED GPU';
    reasonDetail = 'BEACON AI requires a dedicated graphics processor (NVIDIA or AMD) with hardware acceleration. This computer uses integrated graphics, which cannot process the AI model fast enough.';
    specLines = `<div class="hw-spec-line"><span class="hw-spec-label">DETECTED GPU</span><span class="hw-spec-val hw-spec-fail">${gpuName}</span></div>
                 <div class="hw-spec-line"><span class="hw-spec-label">REQUIRED GPU</span><span class="hw-spec-val">NVIDIA or AMD dedicated</span></div>
                 <div class="hw-spec-line"><span class="hw-spec-label">SYSTEM RAM</span><span class="hw-spec-val">${ramGB}</span></div>`;
  }

  // Remove any existing overlays
  hideConnectingOverlay();
  _destroyWarmupElements();
  const old = document.getElementById('hardwareInsufficientOverlay');
  if (old) old.remove();

  const overlay = document.createElement('div');
  overlay.id = 'hardwareInsufficientOverlay';
  overlay.className = 'beacon-warmup-overlay hardware-insufficient-overlay';
  overlay.style.display = 'flex';
  overlay.innerHTML = `
    <div class="warmup-ov-header">
      <div class="warmup-ov-bars"><span></span><span></span><span></span><span></span><span></span></div>
      <div class="warmup-ov-status" style="color:var(--amber-dim)">HARDWARE CHECK FAILED</div>
      <div class="warmup-ov-bars"><span></span><span></span><span></span><span></span><span></span></div>
    </div>
    <div class="warmup-ov-title" style="color:var(--amber-dim)">BEACON:UNAVAILABLE</div>
    <div class="warmup-ov-desc">${reasonTitle}</div>
    <div class="hw-specs-box">
      ${specLines}
    </div>
    <div class="warmup-ov-tip">
      <span class="warmup-ov-tip-label">WHY?</span>
      <span class="warmup-ov-tip-text">${reasonDetail}</span>
    </div>
    <div class="warmup-ov-footer">Library · COMMS · Workspace · Ham Radio · Settings are fully available</div>`;

  const chatCont = document.getElementById('chatContainer');
  if (chatCont && chatCont.parentNode) {
    chatCont.parentNode.insertBefore(overlay, chatCont.nextSibling);
    const header = document.querySelector('.app-header');
    if (header) {
      overlay.style.top = header.offsetHeight + 'px';
    }
  }
  if (chatCont) chatCont.style.filter = 'blur(6px) saturate(0.5)';

  // Update status indicators
  if (statusDot) statusDot.className = 'status-dot error';
  if (statusText) {
    statusText.className = 'status-text error';
    statusText.textContent = 'BEACON:UNAVAILABLE';
  }
  const welcomeTitle = document.querySelector('.welcome-title');
  if (welcomeTitle) {
    welcomeTitle.textContent = 'BEACON:UNAVAILABLE';
    welcomeTitle.style.color = 'var(--amber-dim)';
  }
  if (sendBtn) sendBtn.disabled = true;
  if (userInput) userInput.placeholder = 'AI unavailable — hardware does not meet requirements';
}

async function _checkHardwareGate() {
  /** Check /api/status for engine.aiDisabled flag.
   *  Returns true if AI is disabled (caller should skip maintainConnection). */
  try {
    const res = await fetch('/api/status', { signal: AbortSignal.timeout(8000) });
    if (!res.ok) return false;
    const data = await res.json();
    if (data.engine && data.engine.aiDisabled) {
      _showHardwareInsufficientOverlay(
        data.engine.aiDisabledReason || 'no_gpu',
        data.engine
      );
      return true;
    }
  } catch {
    // Server not up yet or network issue — don't gate, let normal flow handle it
  }
  return false;
}
...
async function maintainConnection(showOverlay = true) {
  ...
  const POLL_FAST = 4000;  // Slightly slower since generate-ping takes up to 5s
  const POLL_SLOW = 15000;

  let _lastState = null;
  let _errorTicks = 0; // Debounce errors during slow USB load
  let _confirmedOnline = false; // Once online, skip expensive generate-ping

  while (true) {
    let rawState;
    if (_confirmedOnline) {
      // Lightweight health check: model was already proven ready, just verify
      // Ollama is still running and model is still in ps
      rawState = await _checkConnectionLight();
    } else {
      rawState = await checkConnection();
    }
    
    // Debounce the error state: Ollama can temporarily time out during heavy USB I/O
    let currentState = rawState;
    if (rawState === 'error' && (_lastState === 'warming_up' || _lastState === null || _lastState === '')) {
      _errorTicks++;
      if (_errorTicks < 5) { // Wait up to ~20 seconds before declaring error
        currentState = _lastState === null || _lastState === '' ? 'warming_up' : _lastState;
      }
    } else {
      _errorTicks = 0;
    }
    ...
    // Track confirmed-online to avoid future generate-pings
    if (currentState === 'online') _confirmedOnline = true;
    if (currentState === 'error') _confirmedOnline = false;
...
// ── Message Rendering ─────────────────────────────────────
function renderMessage(role, content, streaming = false, thinking = null) {
  ...
  // TTS button for BEACON messages (native SpeechSynthesis — zero deps, instant playback)
  ...
  // Reconstruct think block for restored/loaded assistant messages with thinking
  if (!streaming && role === 'assistant' && thinking) {
    const body = msgEl.querySelector('.message-body');
    if (body) {
      const thinkEl = document.createElement('div');
      thinkEl.className = 'think-block complete';
      thinkEl.innerHTML =
      ...
      thinkEl.querySelector('.think-body').textContent = thinking;
      thinkEl.querySelector('.think-header').addEventListener('click', function() {
        thinkEl.classList.toggle('expanded');
      });
      // Wrap existing rendered content in .message-response for TTS isolation
      const responseDiv = document.createElement('div');
      responseDiv.className = 'message-response';
      responseDiv.innerHTML = body.innerHTML;
      body.innerHTML = '';
...
function toggleTTS(btn) {
  ...
  // Extract text from .message-response (skips think block) or fall back to full body
  const responseEl = body.querySelector('.message-response');
  const text = responseEl
    ? (responseEl.innerText || responseEl.textContent || '')
...
function _ttsSpeak(text, btn) {
  ...
  // Premium voice selection: ranked priority across all platforms
  const voice = _ttsBestVoice();
...
// Ranked voice priority: best → worst. First match wins.
let _ttsCachedVoice = null;
function _ttsBestVoice() {
  if (_ttsCachedVoice) return _ttsCachedVoice;
  const voices = window.speechSynthesis.getVoices();
  if (!voices.length) return null;

  const PRIORITY = [
    // macOS Premium/Enhanced voices (highest quality)
    v => /Samantha.*Premium/i.test(v.name),
    v => /Daniel.*Premium/i.test(v.name),
    v => /Samantha.*Enhanced/i.test(v.name),
    v => /Daniel.*Enhanced/i.test(v.name),
    // macOS standard voices
    v => /^Samantha$/i.test(v.name),
    v => /^Daniel$/i.test(v.name),
    v => /^Alex$/i.test(v.name),
    // Windows Neural / Online Natural voices (Edge/Chrome)
    v => /Microsoft.*Online.*Natural/i.test(v.name) && v.lang.startsWith('en'),
    v => /Microsoft.*(Jenny|Guy|Aria|Davis)/i.test(v.name),
    // Google voices (Chrome-only, high quality)
    v => v.name.includes('Google') && v.lang.startsWith('en'),
    // Any English non-espeak, non-Compact voice
    v => v.lang.startsWith('en') && !/espeak|Compact|Legacy/i.test(v.name) && !v.name.includes('Compact'),
    // Any English voice
  ...
  for (const test of PRIORITY) {
    ...
    if (match) { _ttsCachedVoice = match; return match; }
  }
  _ttsCachedVoice = voices[0];
  return voices[0];
}

// Re-evaluate voice cache when voices change (async loading)
if (window.speechSynthesis) {
  window.speechSynthesis.onvoiceschanged = () => { _ttsCachedVoice = null; };
}
...
function toggleAutoRead() {
  ...
  // Sync settings panel checkbox if it's currently visible
  const settingsChk = document.getElementById('settingAutoRead');
  if (settingsChk) settingsChk.checked = _ttsAutoRead;
...
// When the user unlocks from ANY panel (COMMS, Settings, etc.),
// the chat UI must reactively hide the "unsaved" banner and
// trigger a catch-up save. Without this listener, unlocking via
// COMMS left the chat banner stuck until a page reload.
document.addEventListener('bd-auth-state-changed', (e) => {
  if (e.detail && e.detail.unlocked === true) {
    // Drive unlocked — hide the "Chat not being saved" banner
    if (typeof _hideUnsavedBanner === 'function') _hideUnsavedBanner();
    // Catch-up save: any messages that accumulated while locked
    if (typeof _saveChatState === 'function') _saveChatState();
  } else if (e.detail && e.detail.unlocked === false) {
    // Drive locked — if encryption is on, show the unsaved banner
    if (typeof _isEncryptHistoryEnabled === 'function' &&
        _isEncryptHistoryEnabled() &&
        typeof _showUnsavedBanner === 'function') {
      _showUnsavedBanner();
    }
  }
});
...
async function sendMessage() {
  ...
  sendBtn.disabled = false; // Must be enabled so STOP click events fire
  ...
  try {
    ...
    if (fullContent) {
      const msgObj = { role: 'assistant', content: fullContent };
      if (_thinkContent) msgObj.thinking = _thinkContent;
      ...
      // Update Hardware Diagnostics HUD
      const hudSpeed = document.getElementById('hudSpeed');
      if (hudSpeed) hudSpeed.textContent = tokPerSec !== '?' ? tokPerSec : '0.0';
...
// PRIVACY: Chrome's SpeechRecognition API sends audio to Google's servers by
// default. Since v139 (Aug 2025), Chrome supports processLocally = true, which
// forces ALL speech recognition to run on-device — no audio ever leaves the
// machine. We REQUIRE this mode. If the browser doesn't support local
// processing, the mic button stays hidden (same behavior as Firefox/Safari
// which don't support SpeechRecognition at all). This ensures the product's
// "zero telemetry" and "no data leaves your machine" promises are never broken.
let recognition = null;
let isListening = false;

function initVoiceInput() {
  // Show TTS speed/auto-read controls if speechSynthesis available
  // (TTS is always local — uses built-in browser voices, no network)
  // This runs BEFORE speech recognition checks because TTS has no
  // privacy concerns and should always be available.
  if (window.speechSynthesis) {
  ...
  // Probe for processLocally support — we refuse to enable voice input
  // unless we can guarantee audio stays on-device.
  const probe = new SpeechRecognition();
  if (!('processLocally' in probe)) {
    // Browser supports SpeechRecognition but NOT local processing.
    // Enabling it would send audio to Google — do not expose the button.
    console.info('[VOICE] SpeechRecognition available but processLocally not supported — mic disabled to protect privacy.');
  ...
  recognition.processLocally = true; // CRITICAL: forces on-device processing
  ...
  recognition.onerror = (event) => {
    ...
    if (event.error === 'not-allowed') {
      ...
      showToast('On-device speech recognition is not available for this language. Install the language pack in your browser settings.', 5000);
    }
    stopListening();
  };
...
async function _newConversation() {
  if (isGenerating && _currentAbortController) {
    _currentAbortController.abort();
    // Yield briefly to let the finally block clear state
    await new Promise(r => setTimeout(r, 50));
  }
...
async function loadConversation(convId) {
  ...
  // Already viewing this conversation — skip re-fetch, but still
  // navigate back to chat view if we're in another panel (Workspace, Library, etc.)
...
function _doLoadConversation(conv) {
  ...
  messagesEl.innerHTML = '';
  _showWelcome(false);
  for (const msg of messages) renderMessage(msg.role, msg.content, false, msg.thinking || null);
...
function closeAllPanels(keepSpacer = false) {
  ...
  // Close COMMS overlay
  if (typeof closeComms === 'function') closeComms();
...
function _getActiveViewBtn() {
  ...
  if (toolsPanel && toolsPanel.style.display !== 'none') return 'toolsNavBtn';
  const commsPanel = document.getElementById('commsPanel');
  if (commsPanel && commsPanel.style.display !== 'none') return 'commsNavBtn';
  return 'chatNavBtn';
}
...
function _navigateToChat() {
  ...
  if (typeof closeTools === 'function') {
    const toolsPanel = document.getElementById('toolsPanel');
    if (toolsPanel && toolsPanel.style.display !== 'none') closeTools();
  }
  // Close COMMS overlay
  if (typeof closeComms === 'function') {
    const commsPanel = document.getElementById('commsPanel');
    if (commsPanel && commsPanel.style.display !== 'none') closeComms();
  }
...
function _toggleAccordion(id) {
  ...
  // Close all other accordions to keep the panel clean
...
function _loadSettingsState() {
  ...
  const nightVision = document.getElementById('settingNightVision');
  const diagnosticsHud = document.getElementById('settingDiagnosticsHud');
  ...
  if (devMode) {
    devMode.checked = localStorage.getItem('bd-setting-dev-mode') === 'true';
  }

  // Radio Silence — controlled exclusively via COMMS panel Emissions Control
  // State persisted in localStorage key 'bd-setting-radio-silence'

  // ── COMMS toggle states loaded from localStorage (DOM elements removed — controls in COMMS panel)
  const _telStored = localStorage.getItem('bd-setting-radio-telemetry') === 'true';
  const _gpsStored = localStorage.getItem('bd-setting-gps-position') === 'true';
  const _dispStored = localStorage.getItem('bd-setting-dispatch');
  const _dispVal = _dispStored === null ? true : _dispStored === 'true';
  if (_dispStored === null) localStorage.setItem('bd-setting-dispatch', 'true');

  // Sync COMMS settings to backend on load (ensures hardware state matches UI).
  // When Blackout Protocol is ON, all emissions are forced off — don't send
  // stale localStorage values that would immediately get overridden by
  // _syncBlackoutUI() 300ms later (causing the dispatch toggle to cycle).
  // NOTE: Read from localStorage directly — _initBlackoutProtocol() hasn't run
  // yet (it fires later in the init sequence), so the global _blackoutProtocolOn
  // still has its declaration-time default of `true`.
  const _bpStored = localStorage.getItem('bd-blackout-protocol');
  const _bpOn = _bpStored === null ? true : _bpStored === 'true';
  fetch('/api/comms/config', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      radio_telemetry: _bpOn ? false : _telStored,
      gps_position: _bpOn ? false : _gpsStored,
      dispatch_enabled: _bpOn ? false : _dispVal,
    }),
  }).catch(() => {});
  ...
  if (encryptRow) {
    ...
    if (encryptDesc) {
      encryptDesc.textContent = saveIsOn
  ...
  // Re-sync Blackout Protocol overrides (BP may force toggles + rewrite descriptions)
  _syncBlackoutUI();
  ...
  if (nightVision) {
    const isNv = localStorage.getItem('bd-setting-night-vision') === 'true';
    nightVision.checked = isNv;
    _applyNightVision(isNv);
  }
  
  if (diagnosticsHud) {
    const isHud = localStorage.getItem('bd-setting-diagnostics-hud') === 'true';
    diagnosticsHud.checked = isHud;
    _applyDiagnosticsHud(isHud);
  }
...
// Apply night vision class to body
function _applyNightVision(enabled) {
  if (enabled) document.body.classList.add('theme-red');
  else document.body.classList.remove('theme-red');
}

// Toggle diagnostics HUD visibility
function _applyDiagnosticsHud(enabled) {
...
function _initSettings() {
  ...
  // Radio Silence toggle — moved to COMMS panel Emissions Control (comms.js)

  // ── COMMS toggle handlers removed — controls moved to COMMS Emissions Control ──
  ...
  if (encryptHistory) encryptHistory.addEventListener('change', async () => {
    ...
    if (encryptHistory.checked) {
      // Turning ON encryption — require master password via centralized unlock
  ...
  if (nightVision) nightVision.addEventListener('change', (e) => {
    localStorage.setItem('bd-setting-night-vision', e.target.checked);
    _applyNightVision(e.target.checked);
  });
  
  if (diagnosticsHud) diagnosticsHud.addEventListener('change', (e) => {
    localStorage.setItem('bd-setting-diagnostics-hud', e.target.checked);
    _applyDiagnosticsHud(e.target.checked);
  });

  // Developer Mode toggle
  if (devMode) devMode.addEventListener('change', (e) => {
    localStorage.setItem('bd-setting-dev-mode', e.target.checked);
    // Notify workspace to show/hide THE ENGINE
    document.dispatchEvent(new CustomEvent('blackout:devmode-changed', { detail: e.target.checked }));
  });
  ...
  if (engineTier) engineTier.addEventListener('change', async () => {
    ...
    showToast('Switching engines... please wait', 30000);
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        // Poll /api/settings/tier/status until swap completes or fails
        // (FRAG-006 remediation: replaces blind 12s wait with active polling)
        const maxPolls = 60;  // 60 × 2s = 120s max wait
        let done = false;
        for (let i = 0; i < maxPolls; i++) {
          await new Promise(r => setTimeout(r, 2000));
          try {
            const statusRes = await fetch('/api/settings/tier/status');
            ...
            if (status.state === 'done') {
              ...
              showToast(`Engine switch failed: ${status.error || 'unknown error'}`, 5000);
              done = true;
              break;
            }
            // state === 'swapping' — continue polling
          } catch {
            // Server might be busy — keep polling
          }
        }
        if (!done) {
          showToast('Engine switch timed out — restart the drive to apply', 6000);
        }
      } else {
...
// When any panel locks/unlocks, re-render the conversation list so
// encrypted titles and lock/unlock buttons update instantly.
document.addEventListener('bd-auth-state-changed', () => {
  if (typeof _loadConversationList === 'function') {
    _loadConversationList();
  }
});
...
function _saveChatState(silent = false) {
  ...
  if (autoSaveEnabled) {
    ...
    try {
      ...
      // Overflow — strip thinking content (cosmetic data) and retry
      try {
        const stripped = toSave.map(m => {
          if (m.thinking) { const { thinking, ...rest } = m; return rest; }
          return m;
        });
        sessionStorage.setItem(CHAT_SS_KEY, JSON.stringify(stripped));
      } catch {
        // Still overflowing — reduce to last 25 messages
        try {
          const reduced = toSave.slice(-25).map(m => {
            if (m.thinking) { const { thinking, ...rest } = m; return rest; }
            return m;
          });
          sessionStorage.setItem(CHAT_SS_KEY, JSON.stringify(reduced));
        } catch {
          console.warn('[ChatState] sessionStorage full — cannot save chat state');
        }
      }
    }
  }
...
function _restoreChatState() {
  ...
  try {
    ...
    for (const msg of messages) {
      renderMessage(msg.role, msg.content, false, msg.thinking || null);
    }
  } catch {}
}
...
function _initBlackoutProtocol() {
  ...
  if (_blackoutProtocolOn) {
    // BP ON: always force network offline + encrypt history
    _onlineMode = false;
    try { localStorage.setItem('bd_online_mode', '0'); } catch {}
    localStorage.setItem('bd-setting-network-lock', 'true');
    localStorage.setItem('bd-setting-encrypt-history', 'true');
  } else {
    // BP OFF: restore the user's previous Network Lock setting
    const nlStored = localStorage.getItem('bd-setting-network-lock');
    const networkLocked = nlStored === null ? true : nlStored === 'true';
    _onlineMode = !networkLocked;
    try { localStorage.setItem('bd_online_mode', _onlineMode ? '1' : '0'); } catch {}
  }
...
function _syncBlackoutUI() {
  ...
  if (badge) {
    if (_blackoutProtocolOn) {
      ...
      badge.title = 'Blackout Protocol: internet blocked, chats encrypted, radio emissions reduced. Manual messaging still works.';
    } else {
      badge.classList.add('protocol-off');
      badge.title = 'Blackout Protocol OFF — click to re-enable';
    }
  }
  ...
  if (_blackoutProtocolOn) {
    ...
    if (nlRow) {
      ...
      const desc = nlRow.querySelector('.settings-label-desc');
      if (desc) desc.textContent = 'Prevents the drive from accessing the internet. BEACON works fully offline — this only needs to be off if you want to download new library content.';
    }
  }
    ...
    if (ehRow) {
      ...
      if (desc) {
        desc.textContent = saveOn
          ? 'Blackout Protocol is ON — your conversations are encrypted automatically. You can change your password in Settings → System & Data.'
          : 'Enable Save Chat History above to use encryption.';
      }
    }
  } else {
  ...
  // 5. Sync COMMS panel — emission toggles now live exclusively in the COMMS Intel Dashboard.
  // Re-render if panel is open so toggles reflect Blackout Protocol force-off state.
  if (typeof _commsRenderIntelTab === 'function' && typeof _commsOpen !== 'undefined' && _commsOpen) {
    _commsRenderIntelTab();
  }
  // Also push Blackout Protocol state to backend
  if (_blackoutProtocolOn) {
    clearTimeout(_syncBlackoutUI._timer);
    _syncBlackoutUI._timer = setTimeout(() => {
      fetch('/api/comms/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          radio_telemetry: false,
          gps_position: false,
    ...
    // BP just turned OFF — re-sync emission settings from localStorage to backend
    // so the UI checkboxes and backend state agree
    clearTimeout(_syncBlackoutUI._timer);
    _syncBlackoutUI._timer = setTimeout(() => {
      const tel = localStorage.getItem('bd-setting-radio-telemetry') === 'true';
      const gps = localStorage.getItem('bd-setting-gps-position') === 'true';
      const disp = localStorage.getItem('bd-setting-dispatch') !== 'false'; // default true
      fetch('/api/comms/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function _checkFirstRun() {
  try {
    if (localStorage.getItem('bd_onboarded')) {
      // User was previously onboarded — but the drive may have been re-flashed,
      // resetting server-side EULA state. Silently re-accept if needed.
      fetch('/api/eula/status').then(r => r.json()).then(data => {
        if (!data.accepted) {
          fetch('/api/eula/accept', { method: 'POST' }).catch(() => {});
        }
      }).catch(() => {});
      return;
    }
...
function dismissFirstRun() {
  ...
  // Persist EULA acceptance server-side (gates API until accepted)
  fetch('/api/eula/accept', { method: 'POST' }).catch(() => {});
...
(async function init() {
  ...
  // Restore tools state if it was open (tools.js must be loaded first)
  if (typeof _restoreToolsState === 'function') {
    try {
      _restoreToolsState();
    } catch(e) {}
  }
  ...
  // ── HARDWARE GATE CHECK ───────────────────────────────────
  // Before attempting any Ollama connection, check if the hardware
  // is sufficient. If not, show a permanent overlay and skip all
  // AI engine polling to avoid wasting CPU/RAM.
  const hwDisabled = await _checkHardwareGate();
  if (hwDisabled) {
    // Hardware insufficient — AI is completely disabled.
    // Still run heartbeat so the server stays alive for Library/COMMS/etc.
    setInterval(() => {
      fetch('/api/heartbeat').catch(() => {});
    }, 30000);
    fetch('/api/heartbeat').catch(() => {});
    return; // Exit — do NOT start maintainConnection
  }
...
/** Initialize update section — set current version from /api/status */
(function _initUpdateUI() {
  fetch('/api/status').then(r => r.json()).then(d => {
    const el = document.getElementById('updateCurrentVersion');
    if (el && d.version) el.textContent = 'v' + d.version;
    
    // Populate Diagnostics HUD
    const hudRam = document.getElementById('hudRam');
    const hudGpu = document.getElementById('hudGpu');
    if (hudRam && d.engine && d.engine.detectedRamGB) hudRam.textContent = Math.round(d.engine.detectedRamGB) + ' GB';
    if (hudGpu && d.engine && d.engine.gpuName) {
      hudGpu.textContent = d.engine.gpuName;
      hudGpu.title = d.engine.gpuName; // tooltip for long names
    }
  }).catch(() => {});
})();

/** Check for available updates (calls /api/update/check) */
function _checkForUpdates() {
  ...
  const infoTitle = document.getElementById('updateInfoTitle');
  const infoBody = document.getElementById('updateInfoBody');
  const dlBtn = document.getElementById('updateDownloadBtn');
  const helpText = document.getElementById('updateHelpText');

  // Check if network lock is on
  const nlToggle = document.getElementById('settingNetworkLock');
  if (nlToggle && nlToggle.checked) {
    badge.textContent = 'NETWORK LOCKED';
    badge.style.color = 'rgba(255,100,100,0.7)';
    helpText.textContent = 'Turn off Network Lock first to check for updates.';
    return;
  }
  ...
  badge.style.color = 'var(--amber-dim)';
  ...
  fetch('/api/update/check')
    ...
    .then(data => {
      ...
      if (data.error) {
        ...
        badge.style.color = 'rgba(255,100,100,0.7)';
        helpText.textContent = 'Could not reach update server. Try again later.';
        return;
      }

      if (data.update_staged) {
        badge.textContent = 'UPDATE STAGED';
        badge.style.color = '#4caf79';
        infoBox.style.display = 'block';
        infoTitle.textContent = `Update v${data.latest_version} ready to install`;
        infoBody.textContent = 'Close the drive and relaunch to apply the update. ' +
          (data.changelog || '');
        dlBtn.style.display = 'none';
        helpText.textContent = 'Restart the drive to apply the pending update.';
        return;
      }
      ...
      if (data.update_available) {
        ...
        infoBox.style.display = 'block';
        const sizeMB = data.download_size ? (data.download_size / 1048576).toFixed(1) : '?';
        infoTitle.textContent = `v${data.latest_version} available (${sizeMB} MB)`;
        infoBody.textContent = data.changelog || 'Bug fixes and improvements.';
        dlBtn.style.display = 'block';
        helpText.textContent = 'No personal data is transmitted during the update.';
      } else {
        badge.textContent = 'UP TO DATE';
        badge.style.color = '#4caf79';
        infoBox.style.display = 'none';
        dlBtn.style.display = 'none';
        helpText.textContent = `You're running the latest version.`;
      }
    })
    .catch(() => {
      btn.disabled = false;
      ...
      badge.textContent = 'CHECK FAILED';
      badge.style.color = 'rgba(255,100,100,0.7)';
      helpText.textContent = 'Could not reach update server. Check your connection.';
    });
}

/** Download and stage the update (calls /api/update/download) */
function _downloadUpdate() {
  ...
  helpText.textContent = 'Downloading update... do not close the drive.';
  ...
  fetch('/api/update/download', { method: 'POST' })
    ...
    .then(data => {
      if (data.ok) {
        ...
        dlBtn.textContent = '⬇ Download & Install Update';
        checkBtn.style.display = 'block';
        progressBox.style.display = 'none';
        helpText.textContent = 'Download failed: ' + (data.error || 'Unknown error');
      }
    })
    .catch(() => {
      dlBtn.disabled = false;
      ...
      checkBtn.style.display = 'block';
      progressBox.style.display = 'none';
      helpText.textContent = 'Download request failed. Try again.';
    });
}

/** Poll /api/update/status for download progress */
    ...
    fetch('/api/update/status')
      ...
      .then(state => {
        ...
        if (state.state === 'downloading') {
          ...
          bar.style.width = pct + '%';
          const dlMB = (state.progress / 1048576).toFixed(1);
          const totalMB = (state.total / 1048576).toFixed(1);
          text.textContent = `${dlMB} / ${totalMB} MB (${pct}%)`;
          ...
          progressBox.style.display = 'none';
          infoBox.style.display = 'block';
          infoTitle.textContent = `Update v${state.remote_version} ready`;
          infoBody.textContent = 'Close the drive and double-click START to apply the update. Your files and settings will not be affected.';
          helpText.textContent = 'Restart the drive to apply the update.';
          ...
          dlBtn.textContent = '⬇ Download & Install Update';
          checkBtn.style.display = 'block';
          helpText.textContent = 'Error: ' + (state.error || 'Download failed');
        }
      })
      .catch(() => {});
  }, 800);
...
function _showFactoryWipeModal() {
  const overlay = document.getElementById('wipeDriveConfirmOverlay');
  if (overlay) overlay.style.display = 'flex';
}

async function _executeFactoryWipe() {
  const overlay = document.getElementById('wipeDriveConfirmOverlay');
  if (overlay) overlay.style.display = 'none';

  showToast('Executing factory reset... Do not close the drive.', 10000);

  try {
    const res = await fetch('/api/factory-wipe', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ confirm: 'FACTORY_WIPE' }),
    });
    const data = await res.json();
    if (data.ok) {
      localStorage.clear();
      sessionStorage.clear();
      showToast('Drive wiped successfully. Restarting...', 3000);
      setTimeout(() => window.location.reload(), 2000);
    } else {
      showToast('Factory reset failed: ' + (data.error || 'Unknown error'), 5000);
    }
  } catch (e) {
    showToast('Failed to contact server for wipe.', 5000);
  }
}

function _showWipeCommsModal() {
  const overlay = document.getElementById('wipeCommsConfirmOverlay');
  if (overlay) overlay.style.display = 'flex';
}

async function _executeWipeComms() {
  const overlay = document.getElementById('wipeCommsConfirmOverlay');
  if (overlay) overlay.style.display = 'none';

  showToast('Wiping communications log...', 3000);

  try {
    // Require master password for destructive comms wipe
    const pw = await _requireMasterPassword();
    const res = await fetch('/api/comms/wipe', {
      method: 'POST',
      headers: { 'X-Password': pw },
    });
    const data = await res.json();
    if (data.ok) {
      showToast('Comms log wiped successfully.', 3000);
      if (typeof _commsAllMessages !== 'undefined') {
        _commsAllMessages = [];
        _commsLastMsgId = 0;
        _commsRenderedIds.clear();
        _commsUnread = {};
        try { localStorage.removeItem('bd-comms-unread'); } catch(e) {}
        if (typeof _commsOpen !== 'undefined' && _commsOpen) {
          if (typeof _commsRender === 'function') _commsRender();
        }
      }
    } else {
      showToast('Failed to wipe comms: ' + (data.error || 'Unknown error'), 5000);
    }
  } catch (e) {
    showToast('Failed to contact server.', 5000);
  }
}
