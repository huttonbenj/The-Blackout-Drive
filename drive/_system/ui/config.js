...
(function () {
  ...
  const DEFAULTS = {
    app: {
      ...
      version: '1.0.0',
      tagline: 'Offline AI. Encrypted Vault. Mesh Comms.',
    },
    model: {
    ...
    chat: {
      streamTimeoutMs: 300000,  // 5 min — CPU-only inference can be very slow
  ...
  function flattenConfig(c) {
    return {
      ...
      ollamaHost:    `http://127.0.0.1:${(c.network || {}).ollamaPort || DEFAULTS.network.ollamaPort}`,
